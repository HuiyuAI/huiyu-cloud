...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            JWSObject jwsObject = JWSObject.parse(token.replace(SecurityConstants.JWT_PREFIX, ""));
            String payload = jwsObject.getPayload().toString();
            log.info("AuthGlobalFilter.filter() payload:{}", payload);
            ServerHttpRequest request = exchange.getRequest().mutate().header(SecurityConstants.JWT_PAYLOAD_KEY, payload).build();
