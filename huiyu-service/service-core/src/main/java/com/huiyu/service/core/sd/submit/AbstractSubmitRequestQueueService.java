...
public abstract class AbstractSubmitRequestQueueService<T extends Cmd> {
    ...
    private boolean deductUserPoint(Task task) {
        return userBusiness.updatePoint(task.getUserId(), task.getPoint(), PointOperationSourceEnum.GENERATE_PIC, PointOperationTypeEnum.REDUCE, task.getRequestUuid(), null);
    }
}
